...
class TestEvaluateState:
    ...
    def test_classified_to_verified_sustained(self):
        result = evaluate_target_state(
            ...
            fused_confidence=0.85,
            sensor_type_count=1,
            time_in_current_state_sec=16.0,
            seconds_since_last_sensor=0.0,
        )
        assert result == "VERIFIED"
    ...
    def test_undetected_passthrough(self):
        result = evaluate_target_state(
            ...
            sensor_type_count=0,
            time_in_current_state_sec=0.0,
            seconds_since_last_sensor=100.0,
        )
    ...
    def test_terminal_states_not_regressed(self, terminal_state):
        result = evaluate_target_state(
            ...
            sensor_type_count=0,
            time_in_current_state_sec=0.0,
            seconds_since_last_sensor=999.0,
        )
...
class TestRegression:
    def test_verified_to_classified(self):
        result = evaluate_target_state(
            ...
            sensor_type_count=2,
            time_in_current_state_sec=5.0,
            seconds_since_last_sensor=20.0,
        )
        assert result == "CLASSIFIED"
...
class TestThresholds:
    ...
    def test_all_target_types_have_thresholds(self):
        expected = {"SAM", "TEL", "TRUCK", "CP", "MANPADS", "RADAR", "C2_NODE", "LOGISTICS", "ARTILLERY", "APC"}
...
class TestPipelineGate:
    ...
    def test_gate_does_not_fire_on_detected(self):
        ...
        assistant = api_main.TacticalAssistant()
        sim_state = {"targets": [{"id": 1, "type": "SAM", "state": "DETECTED", "lon": 26.0, "lat": 44.0}]}
    ...
    def test_gate_fires_on_verified(self):
        ...
        sim_state_1 = {"targets": [{"id": 1, "type": "SAM", "state": "DETECTED", "lon": 26.0, "lat": 44.0}]}
        assistant.update(sim_state_1)
        # Second tick: VERIFIED (should trigger ISR pipeline)
        sim_state_2 = {
            "targets": [
                {"id": 1, "type": "SAM", "state": "VERIFIED", "lon": 26.0, "lat": 44.0, "detection_confidence": 0.9}
            ]
        }
