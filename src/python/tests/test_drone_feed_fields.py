...
class TestFovTargetsAndSensorQuality:
    def test_fov_targets_and_sensor_quality(self, sim):
        ...
        for uav in state["uavs"]:
            ...
            assert isinstance(uav["fov_targets"], list), (
                f"UAV {uav['id']} fov_targets is not a list"
            )
            # all entries must be ints
            for tid in uav["fov_targets"]:
                assert isinstance(tid, int), (
                    f"UAV {uav['id']} fov_targets contains non-int: {tid}"
                )
            ...
            assert isinstance(sq, float), (
                f"UAV {uav['id']} sensor_quality is not a float: {type(sq)}"
            )
            assert 0.0 <= sq <= 1.0, (
                f"UAV {uav['id']} sensor_quality out of range: {sq}"
            )
    ...
    def test_fov_targets_contains_detected_targets_in_range(self, sim):
        ...
        for uav_dict in state["uavs"]:
            for tid in uav_dict["fov_targets"]:
                assert tid in detected_ids, (
                    f"UAV {uav_dict['id']} fov_targets contains undetected target ID {tid}"
                )
