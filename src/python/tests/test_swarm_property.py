...
from swarm_coordinator import SENSOR_TYPES, SwarmCoordinator
...
uav_strategy = st.fixed_dictionaries(
    {
        ...
        "sensors": st.lists(st.sampled_from(list(SENSOR_TYPES)), min_size=0, max_size=3, unique=True),
    }
)
...
def build_uavs(uav_dicts):
    ...
    return [make_uav(i, d["mode"], d["x"], d["y"], d["sensors"]) for i, d in enumerate(uav_dicts)]


def build_targets(target_dicts):
    """Assign unique IDs and build target objects with no sensor contributions."""
    return [make_target(100 + i, d["state"], d["type"], d["x"], d["y"], []) for i, d in enumerate(target_dicts)]
...
def test_no_dual_assignment(uav_dicts, target_dicts):
    ...
    assert len(assigned_uav_ids) == len(set(assigned_uav_ids)), f"Dual assignment detected: {assigned_uav_ids}"
...
def test_assignment_count_respects_available_drones(uav_dicts, target_dicts):
    ...
    assert len(orders) <= assignable_count, f"Assigned {len(orders)} drones but only {assignable_count} are assignable"
...
def test_assigned_uav_ids_exist_in_input(uav_dicts, target_dicts):
    ...
    for order in orders:
        assert order.uav_id in valid_uav_ids, f"Order references non-existent UAV id {order.uav_id}"
...
def test_assigned_target_ids_exist_in_input(uav_dicts, target_dicts):
    ...
    for order in orders:
        assert order.target_id in valid_target_ids, f"Order references non-existent target id {order.target_id}"
...
def test_force_flag_still_no_dual_assignment(uav_dicts, target_dicts):
    ...
    assigned_uav_ids = [o.uav_id for o in orders]
    assert len(assigned_uav_ids) == len(set(assigned_uav_ids)), f"Dual assignment on force=True: {assigned_uav_ids}"
