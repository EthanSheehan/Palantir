...
class ISRObserverOutput(BaseModel):
    ...
    alerts: List[str] = Field(
        default_factory=list, description="Immediate alerts for the Strategy Analyst regarding High-Priority Targets"
    )
...
class TacticalPlannerOutput(BaseModel):
    ...
    coas: List[CourseOfAction] = Field(
        ..., description="List of the 3 generated COAs (Fastest, Highest Pk, Lowest Cost)"
    )
...
class DriftAlert(BaseModel):
    ...
    target_classification: TargetClassification = Field(
        ..., description="Target type experiencing confidence degradation"
    )
...
class FlywheelHealthReport(BaseModel):
    ...
    labor_reduction_pct: float = Field(
        ..., description="Estimated reduction in human eyes-on-glass time as a percentage"
    )
...
class ThreatRing(BaseModel):
    ...
    source_track_id: Optional[str] = Field(
        default=None, description="Link back to the ISR track that identified this threat"
    )
...
class BattlespaceManagerOutput(BaseModel):
    mission_path: MissionPath = Field(
        ..., description="The terrain-aware mission path generated for the Tactical Planner"
    )
    active_threat_rings: List[ThreatRing] = Field(
        default_factory=list, description="Currently known threat envelopes in the AO"
    )
...
class TaskingManagerOutput(BaseModel):
    ...
    tasking_orders: List[SensorTaskingOrder] = Field(
        default_factory=list, description="Sensor retasking orders issued (empty if confidence is above threshold)"
    )
    confidence_gap: float = Field(
        ..., ge=0.0, le=1.0, description="How far below the confidence threshold the triggering detection is"
    )
...
class SITREPQuery(BaseModel):
    ...
    context_nominations: Optional[List[TargetNomination]] = Field(
        default=None, description="Current Strike Board state"
    )
...
class SynthesisQueryOutput(BaseModel):
    ...
    key_threats: List[str] = Field(
        default_factory=list, description="Bulleted list of the most critical current threats"
    )
    recommended_actions: List[str] = Field(default_factory=list, description="Suggested next steps for the CO")
    data_sources_consulted: List[str] = Field(
        default_factory=list, description="Which data feeds contributed to this SITREP"
    )
...
class PatternAnalyzerOutput(BaseModel):
    ...
    predictive_alerts: List[str] = Field(
        default_factory=list, description="Forward-looking alerts triggered by pattern deviations"
    )
