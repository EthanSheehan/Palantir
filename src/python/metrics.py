"""Prometheus-compatible metrics for Grid-Sentinel C2.
...
def generate_metrics_text() -> str:
    ...
    _histogram(
        "grid_sentinel_tick_duration_seconds",
    ...
    _gauge(
        "grid_sentinel_connected_clients",
        "Number of currently connected WebSocket clients",
        s.connected_clients,
    )

    _counter(
        "grid_sentinel_detection_events",
        "Total detection events from simulation engine",
        s.detection_events_total,
    )

    _counter(
        "grid_sentinel_hitl_approvals",
        "Total HITL nomination approvals",
        s.hitl_approvals_total,
    )

    _counter(
        "grid_sentinel_hitl_rejections",
        "Total HITL nomination rejections",
        s.hitl_rejections_total,
    )

    _gauge(
        "grid_sentinel_targets_active",
        "Number of active (non-destroyed) targets",
        s.targets_active,
    )

    _gauge(
        "grid_sentinel_drones_active",
        "Number of operational drones",
        s.drones_active,
    )

    # Autonomy level as one-hot gauges with label
    lines.append("# HELP grid_sentinel_autonomy_level Current autonomy level (1 = active)")
    lines.append("# TYPE grid_sentinel_autonomy_level gauge")
