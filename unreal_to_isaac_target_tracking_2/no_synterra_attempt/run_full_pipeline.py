"""
Full Automated Pipeline: DEM + Satellite → Terrain USD → Isaac Sim Terminal Dive

Runs everything end-to-end:
  1. Build terrain mesh from DEM + satellite GeoTIFFs
  2. Launch Isaac Sim
  3. Convert OBJ → USD, place truck, apply labels
  4. Run terminal dive guidance
  5. Start WSL2 GPU YOLO watcher
  6. Start Windows live viewer
...
ISAAC_SIM = r"C:\isaac-sim"
ISAAC_PYTHON = os.path.join(ISAAC_SIM, "python.bat")

# ============================================================
# STEP 1: Build terrain mesh
# ============================================================
print("=" * 60)
print("  STEP 1: Building terrain mesh from DEM + satellite")
print("=" * 60)

result = subprocess.run(
    [sys.executable, os.path.join(BASE, "build_terrain_mesh.py")],
    cwd=BASE,
    capture_output=True,
    text=True,
    timeout=300,
...
for f in ["terrain_mesh.obj", "terrain_texture.png", "metadata.json"]:
    ...
    if not os.path.exists(path):
        ...
        sys.exit(1)
print("Terrain mesh ready!\n")

# ============================================================
# STEP 2: Launch Isaac Sim with automated scene setup
# ============================================================
print("=" * 60)
print("  STEP 2: Launching Isaac Sim + scene setup + terminal dive")
print("=" * 60)

# Write a combined Isaac Sim script that does everything
isaac_script = os.path.join(BASE, "_auto_isaac.py")
with open(isaac_script, 'w') as f:
    f.write('''"""Auto-generated: Convert terrain, place truck, run terminal dive."""
...
print(f"Isaac Sim auto-script written to: {isaac_script}")

# Write a startup extension script that Isaac Sim will auto-execute
startup_script = os.path.join(BASE, "_startup.py")
...
# ============================================================
# STEP 3: Launch Isaac Sim with auto-exec
# ============================================================
print("\nLaunching Isaac Sim with auto-exec...")
isaac_proc = subprocess.Popen(
    [os.path.join(ISAAC_SIM, "isaac-sim.bat"),
     f"--exec", f'"{startup_script}"'],
    cwd=ISAAC_SIM,
)

# Wait for Isaac Sim to boot
print("Waiting for Isaac Sim to start (this takes 1-2 minutes)...")
ready_flag = os.path.join(BASE, "_ready.flag")
if os.path.exists(ready_flag):
    os.remove(ready_flag)

# ============================================================
# STEP 4: Start WSL2 GPU YOLO watcher
# ============================================================
print("\nStarting WSL2 GPU YOLO watcher...")
wsl_script = os.path.join(BASE, "WSL2_yolo_gpu_inference.py").replace("C:\\", "/mnt/c/").replace("\\", "/")
wsl_proc = subprocess.Popen(
    ["wsl.exe", "-d", "Ubuntu-22.04", "--", "python3", wsl_script],
)

# ============================================================
# STEP 5: Start Windows live viewer
# ============================================================
...
print("\n" + "=" * 60)
print("  PIPELINE RUNNING — FULLY AUTOMATED")
print("=" * 60)
print(f"  Isaac Sim: PID {isaac_proc.pid}")
print(f"  WSL2 YOLO: PID {wsl_proc.pid}")
print(f"  Viewer:    PID {viewer_proc.pid}")
print()
print("  Isaac Sim will auto-load terrain, place truck,")
print("  start playback, and run the terminal dive.")
print()
print("  If --exec doesn't work, paste this in Script Editor:")
print(f"    exec(open(r'{isaac_script}').read())")
print("  Then hit Play.")
print()
