...
BASE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "output_guidance")
...
try:
    while True:
        ...
        direction = gt_meta.get('direction', 'FWD')
        pass_num = gt_meta.get('pass_num', 0)

        cv2.rectangle(img, (0, 0), (w, 55), (0, 0, 0), -1)
        cv2.putText(img, f"PITCH GUIDANCE | {direction} 50kph | Pass #{pass_num}",
                   (5, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1)

        state_color = (0, 255, 0) if state == 'TRK' else (0, 255, 255)
        cv2.putText(img, f"State: {state} | Pitch: {pitch:.1f} deg | err_y: {error_y:.0f}px",
                   (5, 32), cv2.FONT_HERSHEY_SIMPLEX, 0.45, state_color, 1)

        avg_ms = total_ms / frame_count
        fps = 1000 / avg_ms if avg_ms > 0 else 0
        cv2.putText(img, f"GT:{len(gt_boxes)} | YOLO:{yolo_count} | GPU:{ms:.0f}ms (~{fps:.0f}FPS)",
        ...
        cv2.putText(img, f"Offset: {offset_m:+.0f}m from target",
                   (5, h - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (180, 180, 180), 1)
