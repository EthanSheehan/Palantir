...
Camera pitch adjusts to keep the detected target centered vertically. Drone path is a straight line at 30m, 50kph. A 3D drone STL mesh follows the camera in the viewport.
...
│    ├─ Reads gt_data.json for GT boxes + flight metadata  │
│    ├─ Draws ALL HUD from scratch (no image compositing)  │
...
| `WSL2_yolo_gpu_inference.py` | WSL2 terminal | GPU YOLO + reads GT JSON + draws all HUD |
...
```bash
cd C:\Users\victo\Downloads\unreal_to_isaac_target_tracking_2\scripts\guidance_2
...
## V2 Key Change: JSON Sidecar

Instead of compositing GT frames (which caused HUD phasing), Isaac Sim now writes a `gt_data.json` alongside each raw frame:

```json
{
  "boxes": [
    {"x_min": 253, "y_min": 207, "x_max": 396, "y_max": 286, "label": "truck", "occ": 0.0}
  ],
  "meta": {
    "pitch": 52.3,
    "error_y": -15.0,
...
The WSL2 watcher reads this JSON and draws GT boxes + all HUD text itself. No image compositing = no flicker.
...
| YOLO detects 0 at distance | YOLOv11n not trained for aerial | Expected — GT does steering |
| libpng errors in WSL2 | File read/write contention | Non-fatal, script handles retries |
| JSON read errors | Isaac Sim writing while WSL2 reads | Caught by try/except, uses last good data |
