"""
Drone Flyover Target Detection — Straight-line pass at 30m altitude, 50kph

The drone flies a straight-line surveillance pass over the target:
  1. Starts 500m in front of target at 30m altitude
...
START_OFFSET = 10000.0       # cm = 500m in front
OVERSHOOT = 10000.0          # cm = 100m past target

# 50 kph = 50000 cm / 3600 s = 1388.9 cm/s
SPEED_CM_PER_SEC = 1388.9  # 50kph in cm/s
...
async def flyover_loop():
    ...
    print("=" * 55)
    print("  DRONE FLYOVER — 50kph @ 30m")
    print(f"  Target: {target_name}")
    print("  GREEN = Ground Truth | RED = YOLO")
    print("  To stop: RUNNING = False")
    print("=" * 55)

    # Cache last YOLO/GT results for smooth rendering
    yolo_dets = []
    gt_boxes = []
    yolo_ms = 0
    YOLO_INTERVAL = 5  # Run YOLO every N frames (camera moves every frame)

    while RUNNING and frame < 10000:
        # Move camera EVERY update tick for smooth motion
        now = time.time()
        dt = min(now - last_time, 0.2)  # cap at 200ms
        last_time = now
        ...
        # Let renderer update (smooth camera movement)
        await omni.kit.app.get_app().next_update_async()

        # Only run Replicator + YOLO every N frames
        if frame % YOLO_INTERVAL == 0:
            ...
            if rgb_data is not None:
                ...
                # Direction arrow
                dir_label = "→ FWD 50kph" if direction == 1 else "← REV 50kph"
        ...
        if frame % 50 == 0:
            dist_m = current_offset / 100
            print(f"[Frame {frame}] offset={dist_m:+.0f}m GT:{len(gt_boxes)} YOLO:{len(yolo_dets)}")
