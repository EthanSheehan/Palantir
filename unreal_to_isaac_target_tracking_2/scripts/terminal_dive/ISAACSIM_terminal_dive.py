...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "output_terminal_dive")
...
def set_drone_and_camera(pos, fly_dir, pitch_deg):
    """Set camera AND drone mesh with SAME pitch — camera fixed to body."""
    ...
    while RUNNING and frame < 20000:
        ...
        if phase == PHASE_CRUISE:
            ...
            if dist_to_target <= DIVE_TRIGGER_DIST:
                phase = PHASE_DIVE
                print(f"[Frame {frame}] === DIVE INITIATED === dist={dist_to_target/100:.0f}m alt={drone_pos[2]/100:.0f}m")

        elif phase == PHASE_DIVE:
            # Descend based on pitch angle
            descent_rate = SPEED_CM_PER_SEC * np.sin(np.radians(drone_pitch))
            drone_pos[2] -= descent_rate * dt

            # Check for impact
            alt_above_target = drone_pos[2] - target_pos[2]
        ...
        if frame % 50 == 0:
            alt_m = (drone_pos[2] - target_pos[2]) / 100
