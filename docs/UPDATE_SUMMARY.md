...
- `docs/RUNBOOK.md` — Added amc-grid.sh auto-kill note, updated serve commands to `serve.py`
...
Updated AMC-Grid C2 documentation to reflect current codebase state. All documentation is now generated from source code and configuration files, ensuring accuracy and freshness.
...
- **Contents**:
  ...
  - 11-row script reference table (amc-grid.sh, pytest, npm scripts)
...
1. **amc-grid.sh** — Extracted all launcher commands and scripts
...
- `amc-grid.sh` (2026-03-17) — Latest launcher
...
| Command Accuracy | 100% | All scripts verified in amc-grid.sh |
...
**Verification**: All content verified against source files (amc-grid.sh, requirements.txt, .env.example, source code structure)
