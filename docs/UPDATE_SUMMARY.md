...
- `docs/RUNBOOK.md` — Added grid-sentinel.sh auto-kill note, updated serve commands to `serve.py`
...
Updated Grid-Sentinel C2 documentation to reflect current codebase state. All documentation is now generated from source code and configuration files, ensuring accuracy and freshness.
...
- **Contents**:
  ...
  - 11-row script reference table (grid-sentinel.sh, pytest, npm scripts)
...
1. **grid-sentinel.sh** — Extracted all launcher commands and scripts
...
- `grid-sentinel.sh` (2026-03-17) — Latest launcher
...
| Command Accuracy | 100% | All scripts verified in grid-sentinel.sh |
...
**Verification**: All content verified against source files (grid-sentinel.sh, requirements.txt, .env.example, source code structure)
