# Project Context: Grid-Sentinel C2

**Goal:** High-fidelity recreation of the multi-domain Command and Control (C2) capabilities (a "decision-centric" orchestration model) utilizing specialized AI agents to automate the F2T2EA kill chain.

**Physics constraints:** Decision-logic modeling, sensor fusion (UAV, satellite, SIGINT), time-latency constraints for kill-chain, and ontology mapping.

**Project Name:** Antigravity
